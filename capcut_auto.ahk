...
StartProcess(*)
{
    ...
    Loop totalRounds
    {
        ...
        ; Thêm SetKeyDelay để đảm bảo từng ký tự được gõ chính xác
        SetKeyDelay 50, 50  ; 50ms giữa các lần nhấn phím, 50ms giữa các ký tự
        Send "!n"           ; Alt+N focus vào ô File name
        Sleep 400
        Send "^a"           ; Chọn toàn bộ
        Sleep 150
        ; Dùng SendInput thay vì SendText để đảm bảo độ tin cậy
        SendInput "{Text}" fullPath
        Sleep 400
        Send "{Enter}"
        Sleep 1500
        ; Reset lại key delay về mặc định
        SetKeyDelay -1, -1
